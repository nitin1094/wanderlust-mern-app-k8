...
-  Setup <a href="kubeadm.md"><u> Kubeadm </a></u>
...
```bash
git clone -b main https://github.com/nitin1094/wanderlust-mern-app-k8.git
```
...
```
![Update context](assets/context%20wanderlust.png)
...
```
![Alt text](assets/get-coredns.png)
...
![replica 4](assets/edit-coredns.png)
...
vi .env.docker
```
![IP](assets/frontend.env.docker.png)

#
9) Build frontend docker image : 
```bash
docker build -t <your-dockerhub-username>/frontend-wanderlust:v2.1.8 .
```
![Dockerfile frontend](assets/docker%20frontend%20build.png)
...
![Backend env file](assets/backend.env.docker.png)

#
12) Build backend docker image : 
```bash
docker build -t <your-dockerhub-username>/backend-wanderlust:v2.1.8 .
```
![Backend dockerfile](assets/docker%20backend%20build.png)
...
16) Apply manifests file the below order:
    ...
    ```
    ![Peristent volume](assets/pv.png)

    - Create persistent volume Claim :
    ```bash
    kubectl apply -f persistentVolumeClaim.yaml 
    ```
    ![Peristent volume Claim](assets/pvc.png)
...
kubectl get all
```
![all deployments and services](assets/all-deps.png)
